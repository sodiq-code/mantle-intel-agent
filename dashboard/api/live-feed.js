 ...
 * Streams real-time Mantle block data via Server-Sent Events (SSE).
 * Polls Mantle RPC directly from the edge — no static JSON.
 *
...
async function fetchLatestBlocks(rpcUrl, count = 20) {
...
function detectAnomalies(features) {
  ...
  for (let i = 0; i < features.length; i++) {
    ...
    if (methodCount >= 2 || score >= 0.4) {
      ...
      const hash = await_sha256(`${f.block_num}|${atype}|${f.tx_count}|${f.total_value_mnt}`);
...
function await_sha256(str) {
  // Simple deterministic hash for edge runtime (no crypto module in edge)
  let hash = 0;
...
function buildTitle(type, feat, knownWallets, conf) {
...
function buildInsight(type, feat, knownWallets) {
...
export default async function handler(req) {
  ...
  if (stream) {
    // SSE stream — push one update every 12 seconds (Mantle block time ~2s, batch every 6 blocks)
    ...
    (async () => {
      try {
        for (let i = 0; i < 5; i++) {  // 5 updates max per connection (~60s)
